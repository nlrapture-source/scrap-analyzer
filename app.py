...
# 2. Ανέβασμα φωτογραφίας από τον χρήστη
...
    
    # 3. Προετοιμασία του AI Prompt με τις ΝΕΕΣ κατηγορίες
...
    """
    
    # 4. Κλήση του Gemini AI
    try:
        response = model.generate_content([prompt, image])
        
        # Καθαρισμός του κειμένου για να πάρουμε σωστό JSON
...
            
            # 5. Δημιουργία του Διαγράμματος Πίτας (Pie Chart)
            labels = list(data.keys())
            sizes = list(data.values())
            colors = ['#ff9999','#66b3ff','#99ff99'] # Χρώματα για κάθε κομμάτι
            
            fig, ax = plt.subplots()
...
            st.pyplot(fig)
            
            # Εμφάνιση των αποτελεσμάτων σε λίστα
